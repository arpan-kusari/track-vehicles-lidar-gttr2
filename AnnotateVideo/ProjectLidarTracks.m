%   Identify lidar detections that occur near the time of this frame.
    TheseLidarTracks = (abs(LidarTracks.TimeCs - TimeCs) <= LidarTimeCsTol & LidarTracks.ZTC < LidarRangeThresh);
    LidarTracksSubset = LidarTracks(TheseLidarTracks,:);
    nLDet = height(LidarTracksSubset);
    
%   Project each one.
	nld = 0;
	while (nld < nLDet)
        nld = nld + 1;
        col = LidarTracksSubset.col(nld);
        row = LidarTracksSubset.row(nld);
        ZC = LidarTracksSubset.ZTC(nld);
        nObjCnt = LidarTracksSubset.Track(nld);
        nL2 = LidarTracksSubset.Lidar(nld);
...
%       Get the subset for this TimeCs.        
        LDVertexColsSubset = LDVertexCols(TheseLidarTracks,:);
        LDVertexRowsSubset = LDVertexRows(TheseLidarTracks,:);
